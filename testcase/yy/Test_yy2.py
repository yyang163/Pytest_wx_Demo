...
@allure.epic("杨杨项目2")
@allure.feature('自测功能2')
class Test_yy2:

    # @pytest.mark.parametrize('args', [1, 2, 3])
    # @pytest.mark.parametrize('args', {'a': 1, 'b': 2, 'c': 3})
    @allure.story("自测加法用例2")
    @pytest.mark.parametrize('case', get_case_array2())
    def test_yyy2(self, case, yy_fixture):
        # print(f'------------{case}------------')
        # 业务逻辑
        # 断言：判断 （a + b）* c 是否等于 res
        assert (case['a'] + case['b']) * case['c'] == case['res']

    @allure.story("自测2222用例2")
    @pytest.mark.skip
    @pytest.mark.parametrize('case', get_case_array2())
    def test_yy22(self, case):
        # print(f'------------{case}------------')
        # 业务逻辑
        # 断言：判断 （a + b）* c 是否等于 res
        assert (case['a'] + case['b']) * case['c'] == case['res']
